...
const EXTRAS_OPTIONS = [
  { id: 'Airport transfer', label: 'Airport transfer', Icon: IconTransfer, precio: 25 },
  { id: 'Meal plan', label: 'Meal plan', Icon: IconMeal, precio: 20 },
  { id: 'Yoga sessions', label: 'Yoga sessions', Icon: IconYoga, precio: 15 },
  { id: 'Massage', label: 'Massage', Icon: IconMassage, precio: 30 },
  { id: 'Equipment rental', label: 'Equipment rental', Icon: IconEquipment, precio: 35 },
  { id: 'Photo & video pack', label: 'Photo & video pack', Icon: IconCamera, precio: 40 },
]

const PRECIO_NOCHE = 35
const PRECIO_HORA = 45

function calcularPrecio(noches, horas, extras) {
  const base = noches * PRECIO_NOCHE + horas * PRECIO_HORA
...
function PasoActividad({ actividad, setActividad }) {
  return (
    <div>
      <h2 className="font-display text-2xl text-slate-900 dark:text-white sm:text-3xl">Choose your activity</h2>
...
function PasoNoches({ noches, setNoches }) {
  return (
    <div>
      <h2 className="font-display text-2xl text-slate-900 dark:text-white sm:text-3xl">How many nights?</h2>
...
function PasoHoras({ horas, setHoras }) {
  return (
    <div>
      <h2 className="font-display text-2xl text-slate-900 dark:text-white sm:text-3xl">Class hours</h2>
      <p className="mt-2 text-sm text-slate-600 dark:text-slate-300">How many hours of classes do you want?</p>
      <div className="mt-6 flex flex-wrap justify-center gap-3">
        {HORAS_OPTIONS.map((h) => (
          <button
            key={h}
...
function PasoExtras({ extras, toggleExtra }) {
  return (
    <div>
      <h2 className="font-display text-2xl text-slate-900 dark:text-white sm:text-3xl">Add extras</h2>
          ...
          return (
            ...
            >
              ...
              <div className="flex-1">
                ...
                <p className="text-xs text-slate-500 dark:text-slate-400">${e.precio} USD</p>
              </div>
              <div
                className={`flex h-5 w-5 items-center justify-center rounded-md border-2 text-xs transition-colors ${
                  selected
...
function PasoResumen({ actividad, noches, horas, extras, datosUsuario, setDatosUsuario, generarLinkWhatsApp }) {
  const precioTotal = calcularPrecio(noches, horas, extras)
  const extrasSeleccionados = EXTRAS_OPTIONS.filter((e) => extras.includes(e.id))

  return (
    <div>
      <h2 className="font-display text-2xl text-slate-900 dark:text-white sm:text-3xl">Your trip summary</h2>
            ...
            {extrasSeleccionados.length > 0 ? (
              <ul className="mt-1 space-y-0.5">
                {extrasSeleccionados.map((e) => (
                  <li key={e.id} className="flex items-center gap-2 text-sm text-slate-700 dark:text-slate-300">
      ...
      <section className="bg-background-light py-16 sm:py-20 lg:py-28 dark:bg-background-dark">
        <div className="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8">
          <div className="mb-8 text-center">
            ...
            <h1 className="mt-3 font-display text-2xl text-slate-900 dark:text-white sm:text-3xl md:text-4xl lg:text-5xl">
          ...
          <div className="rounded-2xl border border-slate-200 bg-white/80 p-4 shadow-sm backdrop-blur dark:border-white/10 dark:bg-surface-dark sm:p-6 lg:p-8">
            {paso === 1 && <PasoActividad actividad={actividad} setActividad={setActividad} />}
            {paso === 2 && <PasoNoches noches={noches} setNoches={setNoches} />}
            {paso === 3 && <PasoHoras horas={horas} setHoras={setHoras} />}
            {paso === 4 && <PasoExtras extras={extras} toggleExtra={toggleExtra} />}
            {paso === 5 && (
              <PasoResumen
                actividad={actividad}
                noches={noches}
            ...
            <div className="mt-8 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
              {paso > 1 ? (
                <button
                  ...
                  className="inline-flex min-h-12 w-full items-center justify-center rounded-full border border-slate-300 px-5 py-3 text-sm font-medium text-slate-700 transition-colors hover:border-primary hover:text-primary dark:border-slate-600 dark:text-slate-200 sm:w-auto"
                >
                  Back
                </button>
              ) : (
              ...
              <div className="flex w-full flex-col gap-3 sm:w-auto sm:flex-row sm:items-center">
                ...
                {paso < 5 && (
                  <button
                    ...
                    disabled={!canNext}
                    className="inline-flex min-h-12 w-full items-center justify-center rounded-full bg-primary px-6 py-3 text-sm font-medium text-white transition-colors hover:bg-primary/90 disabled:cursor-not-allowed disabled:opacity-40 sm:w-auto"
                  >
                    Next
                  </button>
                )}
