        ...
        <section className="bg-[#031015] px-4 py-16 sm:px-6 sm:py-20 lg:px-8 lg:py-28">
            <div className="mx-auto max-w-7xl">
                <div className="mb-12 text-center sm:mb-16">
                    <h2 className="mb-4 text-2xl font-bold uppercase tracking-tighter text-white sm:text-3xl md:text-4xl lg:text-5xl">
                        {t('blogSafariKite.ui.dayInLife')}
                    </h2>
                    <p className="text-base font-light leading-relaxed text-[#9eaeb4] sm:text-lg md:text-xl">
                        {t('blogSafariKite.ui.rhythmDictates')}
                    </p>
                </div>
                <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5">
                    {SAFARI_DATA10.timeline.map((item, i) => (
                        <div key={i} className={`relative flex flex-col overflow-hidden rounded-2xl border p-4 group sm:p-6 
                            ...
                            <h3 className="mb-2 text-lg font-bold uppercase tracking-tight text-white">{t(item.titleKey)}</h3>
                            <p className="text-xs text-[#9eaeb4] leading-relaxed mb-6">{t(item.descKey)}</p>
