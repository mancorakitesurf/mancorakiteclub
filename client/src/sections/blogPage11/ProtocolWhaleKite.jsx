        ...
        <section className="bg-[#05161b] px-4 py-16 sm:px-6 sm:py-20 lg:px-8 lg:py-28">
            <div className="mx-auto grid max-w-7xl grid-cols-1 items-center gap-8 sm:gap-10 lg:grid-cols-2 lg:gap-16">
                ...
                <div className="space-y-8">
                    <div>
                        <h3 className="mb-2 text-2xl font-bold uppercase tracking-tighter italic text-white sm:text-3xl md:text-4xl">{t(timing.titleKey)}</h3>
                        <p className="text-sm font-light leading-relaxed text-[#9eaeb4] sm:text-base">{t(timing.subtitleKey)}</p>
                    </div>
                    <div className="space-y-4">
                        {timing.items.map((item, i) => (
                            <div key={i} className="flex items-start gap-4 rounded-2xl border border-white/5 bg-[#091c22] p-4 sm:p-5">
                                ...
                                <div>
                                    <h4 className="font-bold text-white uppercase tracking-tight text-sm">{t(item.labelKey)}</h4>
                                    <p className="text-xs text-[#9eaeb4] leading-relaxed mt-1">{t(item.textKey)}</p>
                                </div>
                            </div>
                        ))}
                    </div>
                ...
                <div className="relative overflow-hidden rounded-2xl border border-[#5af8fb]/20 bg-[#0e2229] p-6 shadow-[0_0_40px_rgba(90,248,251,0.05)] sm:p-8 md:p-10">
                    ...
                    <div className="flex items-center gap-3 mb-8 border-b border-white/10 pb-4">
                        ...
                        <h3 className="text-lg font-bold uppercase tracking-[0.2em] italic text-white sm:text-xl">{t(protocol.titleKey)}</h3>
                    </div>
                    ...
                    <ul className="space-y-8">
                        {protocol.rules.map((rule, i) => (
                            <li key={i} className="flex items-start gap-5">
                                ...
                                <div>
                                    <strong className={`block mb-1 uppercase tracking-wider text-xs ${rule.color}`}>{t(rule.labelKey)}</strong>
                                    <span className="text-sm text-[#9eaeb4] font-light leading-relaxed">{t(rule.textKey)}</span>
                                </div>
                            </li>
                        ))}
                    </ul>
