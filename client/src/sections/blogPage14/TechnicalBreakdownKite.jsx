                ...
                {CHOICE_DATA14.factors.map((factor, i) => (
                    <div key={i} className={`flex flex-col lg:flex-row items-center gap-12 lg:gap-24 ${factor.isReversed ? 'lg:flex-row-reverse' : ''}`}>
                        <div className="w-full lg:w-1/2 relative">
                            ...
                            {/* Dejé picsum.photos como placeholder tal cual lo tenías */}
                            <img src={`https://picsum.photos/800/1000?random=${i}`} loading="lazy" className="relative z-10 aspect-[4/5] w-full rounded-2xl border border-white/10 object-cover grayscale transition-all duration-700 hover:grayscale-0" alt={t(factor.labelKey)} />
                        </div>
                        ...
                        <div className="w-full lg:w-1/2 space-y-6">
                            <div className="flex items-center gap-4 text-[#5af8fb]">
                                ...
                                <span className="text-sm font-bold tracking-[0.3em] uppercase">{t(factor.labelKey)}</span>
                            </div>
                                ...
                                <div className="mt-8 grid grid-cols-1 gap-4 sm:grid-cols-2">
                                    {factor.zones.map((zone, idx) => (
                                        <div key={idx} className={`rounded-2xl border border-white/5 border-t-2 bg-[#031015] p-4 sm:p-6 ${zone.color}`}>
                                            <p className="text-[10px] uppercase tracking-widest opacity-60 mb-2 font-bold">{t('blogWingVsKite.ui.zone')}</p>
                                            <p className="text-xl font-bold tracking-tight">{zone.value}</p>
                                            <p className="text-[10px] uppercase mt-1 opacity-80">{t(zone.nameKey)}</p>
                                        </div>
                                    ))}
                                </div>
                            )}
