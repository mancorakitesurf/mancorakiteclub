        ...
        <section className="max-w-5xl mx-auto px-6 py-24 space-y-32 bg-[#031015]">
            ...
            <div className="grid grid-cols-1 md:grid-cols-12 gap-12 items-center">
                <div className="md:col-span-5 space-y-6">
                    <h2 className="text-3xl font-bold text-white uppercase tracking-tighter italic">{mechanics.title}</h2>
                    <p className="text-[#9eaeb4] leading-relaxed font-light">{mechanics.body}</p>
                    <ul className="space-y-4 pt-4 border-t border-white/10">
                        {mechanics.bullets.map((bullet, i) => (
                            <li key={i} className="flex items-start gap-3">
                                <span className="material-symbols-outlined text-[#ff9069] text-xl">check_circle</span>
                                <span className="text-sm text-[#e6f6fd] font-light">{bullet}</span>
                            </li>
                        ))}
                    </ul>
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-12 gap-12 items-center">
                <div className="md:col-span-7 order-2 md:order-1 relative h-96 bg-[#132930] rounded-xl overflow-hidden border border-white/10 p-8 flex flex-col justify-between shadow-2xl">
                    ...
                    <div className="relative z-10 space-y-4">
                        ...
                        <h3 className="text-2xl font-bold text-white uppercase italic tracking-tight">{calibration.boxTitle}</h3>
                        <p className="text-sm text-[#9eaeb4] font-light leading-relaxed">{calibration.boxBody}</p>
                    </div>
                    <div className="relative z-10 flex justify-between items-end border-t border-white/10 pt-4">
                        {calibration.arcs.map((arc, i) => (
                            <div key={i} className={arc.side === 'right' ? 'text-right' : ''}>
                                <span className="block text-[10px] font-bold text-[#9eaeb4] uppercase mb-1">{arc.label}</span>
                                <span className={`text-lg font-bold uppercase ${arc.highlight ? 'text-[#5af8fb]' : 'text-white'}`}>{arc.value}</span>
                            </div>
                        ))}
                    </div>
                </div>
                <div className="md:col-span-5 order-1 md:order-2 space-y-6">
                    <h2 className="text-3xl font-bold text-white uppercase tracking-tighter italic">{calibration.title}</h2>
                    <p className="text-[#9eaeb4] leading-relaxed font-light">
                        The relationship between line length and turning radius defines your placement on the wave face. Manipulating this arc dictates vertical projection.
                    </p>
                </div>
            </div>
