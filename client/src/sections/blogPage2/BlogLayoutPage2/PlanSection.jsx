  ...
  return (
    <section className="relative py-48 overflow-hidden">
      <div className="absolute inset-0">
        <img src={plan.image} alt="" className="w-full h-full object-cover scale-110 blur-[2px]" />
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-8">
        <div className="grid md:grid-cols-2 gap-24 items-center">
          ...
          >
            <span className={`absolute top-[-120px] ${textSide === "left" ? "left-0" : "right-0"} text-[220px] font-extrabold text-white/10 leading-none select-none`}>
              {plan.letter}
            </span>

            <p className="uppercase tracking-[0.35em] text-xs text-white/70 mb-6">
              {t('blog7Plans.ui.plan')} {plan.letter}
            </p>
            ...
            <div className="grid grid-cols-2 gap-y-6 gap-x-12 text-sm text-white/80">
              <div>
                <span className="block uppercase tracking-wide text-xs text-white/50">
                  {t('blog7Plans.ui.duration')}
                </span>
                {plan.duration}
              </div>
              <div>
                <span className="block uppercase tracking-wide text-xs text-white/50">
                  {t('blog7Plans.ui.level')}
                </span>
                {plan.level}
              </div>
              <div>
                <span className="block uppercase tracking-wide text-xs text-white/50">
                  {t('blog7Plans.ui.spots')}
                </span>
                {plan.spots}
              </div>
              <div>
                <span className="block uppercase tracking-wide text-xs text-white/50">
                  {t('blog7Plans.ui.vibe')}
                </span>
                {plan.vibe}
              </div>
            </div>
